                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="GET" class="mb-4">
                        ...
                        <div class="flex gap-2">
                            <select name="service_id" class="border-gray-300 rounded dark:bg-dark-700 dark:border-dark-600 dark:text-gray-200">
                            ...
                            <a href="{{ route('admin.commercial.extras.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded dark:bg-dark-700 dark:text-gray-200 dark:hover:bg-dark-600">Limpar</a>
                        </div>
                    </form>
                            ...
                            <table class="min-w-full">
                                ...
                                @foreach ($extras as $e)
                                    <tr class="border-b">
                                        ...
                                        <td class="py-3 pr-4">@if($e->price_type==='percent'){{ $e->price }}%@else R$ {{ number_format($e->price,2,',','.') }}@endif</td>
                                        <td class="py-3 pr-4">{{ $e->is_active ? 'Sim' : 'Não' }}</td>
                                        <td class="py-3 pr-4 text-sm flex gap-3">
                                            <a href="{{ route('admin.commercial.extras.edit', $e) }}" class="inline-flex items-center px-3 py-2 bg-orange-600 text-white rounded hover:bg-orange-700">Editar</a>
                                            <form method="POST" action="{{ route('admin.commercial.extras.destroy', $e) }}" onsubmit="return confirm('Remover extra?');">
                                                @csrf
                                                @method('DELETE')
