    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data" class="space-y-6 js-project-update">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $project->name) }}" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Slug</label>
                                <input type="text" name="slug" value="{{ old('slug', $project->slug) }}" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="service_id" class="mt-1 block w-full border-gray-300 rounded-md">
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}" @selected(old('service_id', $project->service_id) == $service->id)>{{ $service->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative group cursor-pointer w-40 h-40">
                                    ...
                                    @if ($project->cover)
                                        <img id="preview-cover" src="{{ asset($project->cover) }}" alt="Cover" class="w-40 h-40 object-cover rounded border border-gray-200 group-hover:opacity-80 transition">
                                    @else
                            ...
                            <div>
                                ...
                                <input type="text" name="video" value="{{ old('video', $project->video) }}" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="https://...">
                            </div>
                        </div>

                        <div class="flex justify-end">
                            <button class="inline-flex items-center px-5 py-3 bg-orange-600 text-white rounded border border-transparent hover:bg-white hover:text-orange-600 hover:border-orange-600 hover:border-solid text-sm">Salvar Informações</button>
                        </div>
                    </form>
                    ...
                    <div id="challenges-list" class="space-y-3">
                        @foreach ($project->challenges as $challenge)
                            <div id="challenge-{{ $challenge->id }}" class="bg-white p-4 rounded shadow-sm border border-gray-100">
                                <form method="POST" action="{{ route('admin.challenges.update', $challenge) }}" class="grid grid-cols-12 gap-3 js-challenge-update" data-id="{{ $challenge->id }}">
                                    @csrf @method('PUT')
                                    <div class="col-span-5">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Título</label>
                                        <input type="text" name="title" value="{{ $challenge->title }}" class="w-full border-gray-300 rounded-md text-sm" required>
                                    </div>
                                    <div class="col-span-6">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
                                        <input type="text" name="description" value="{{ $challenge->description }}" class="w-full border-gray-300 rounded-md text-sm">
                                    </div>
                                    <div class="col-span-1 flex items-end gap-2 justify-end">
                                        ...
                                        <form method="POST" action="{{ route('admin.challenges.destroy', $challenge) }}" class="js-challenge-destroy" data-id="{{ $challenge->id }}" onsubmit="return confirm('Remover desafio?');">
                                            @csrf @method('DELETE')
                        ...
                        <form method="POST" action="{{ route('admin.projects.challenges.store', $project) }}" class="bg-white p-4 rounded shadow-sm border border-gray-100 js-challenge-store">
                            @csrf
                            <h4 class="font-medium text-gray-800 mb-3">Adicionar novo desafio</h4>
                            <div class="grid grid-cols-12 gap-3 items-end">
                                <div class="col-span-5">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Título</label>
                                    <input type="text" name="title" class="w-full border-gray-300 rounded-md text-sm" placeholder="Novo desafio" required>
                                </div>
                                <div class="col-span-6">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
                                    <input type="text" name="description" class="w-full border-gray-300 rounded-md text-sm" placeholder="Descrição">
                                </div>
                                <div class="col-span-1 flex items-end justify-end">
                                    <button class="inline-flex items-center justify-center px-4 py-3 bg-orange-600 text-white rounded border border-transparent hover:bg-white hover:text-orange-600 hover:border-orange-600 text-sm">
                    ...
                    <div id="solutions-list" class="space-y-3">
                        @foreach ($project->solutions as $solution)
                            <div id="solution-{{ $solution->id }}" class="bg-white p-4 rounded shadow-sm border border-gray-100">
                                <form method="POST" action="{{ route('admin.solutions.update', $solution) }}" class="grid grid-cols-12 gap-3 js-solution-update" data-id="{{ $solution->id }}">
                                    @csrf @method('PUT')
                                    <div class="col-span-5">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Título</label>
                                        <input type="text" name="title" value="{{ $solution->title }}" class="w-full border-gray-300 rounded-md text-sm" required>
                                    </div>
                                    <div class="col-span-6">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
                                        <input type="text" name="description" value="{{ $solution->description }}" class="w-full border-gray-300 rounded-md text-sm">
                                    </div>
                                    <div class="col-span-1 flex items-end gap-2 justify-end">
                                        <button class="inline-flex items-center justify-center px-4 py-3 bg-orange-600 text-white rounded border border-transparent hover:bg-white hover:text-orange-600 hover:border-orange-600 text-sm" title="Atualizar">
                                            <i class="fa-solid fa-rotate-right"></i>
                                        ...
                                        <form method="POST" action="{{ route('admin.solutions.destroy', $solution) }}" class="js-solution-destroy" data-id="{{ $solution->id }}" onsubmit="return confirm('Remover solução?');">
                                            @csrf @method('DELETE')
                                            <button class="inline-flex items-center justify-center px-4 py-3 bg-red-600 text-white rounded hover:bg-red-700 text-sm" title="Apagar">
                                                <i class="fa-regular fa-trash-can"></i>
                                            </button>
                        ...
                        <form method="POST" action="{{ route('admin.projects.solutions.store', $project) }}" class="bg-white p-4 rounded shadow-sm border border-gray-100 js-solution-store">
                            @csrf
                            <h4 class="font-medium text-gray-800 mb-3">Adicionar nova solução</h4>
                            <div class="grid grid-cols-12 gap-3 items-end">
                                <div class="col-span-5">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Título</label>
                                    <input type="text" name="title" class="w-full border-gray-300 rounded-md text-sm" placeholder="Nova solução" required>
                                </div>
                                <div class="col-span-6">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
                                    <input type="text" name="description" class="w-full border-gray-300 rounded-md text-sm" placeholder="Descrição">
                    ...
                    <form method="POST" action="{{ route('admin.projects.processes.store', $project) }}" class="bg-white p-4 rounded shadow-sm border border-gray-100 mb-4 grid grid-cols-12 gap-3 items-end js-process-store" data-update-base="/admin/project-processes/__ID__" data-destroy-base="/admin/project-processes/__ID__" data-images-store-base="/admin/project-processes/__ID__/images" data-image-update-base="/admin/project-images/__ID__">
                        @csrf
                            ...
                            <div id="process-card-{{ $pp->id }}" class="bg-white p-4 rounded shadow-sm border border-gray-200">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button type="button" onclick="openUploadModal('upload-pp-{{ $pp->id }}')" class="inline-flex items-center gap-1 px-4 py-2 bg-orange-600 text-white rounded text-sm hover:bg-orange-700">
                                            <i class="fa-regular fa-image"></i> Adicionar Imagens
                                        </button>
                                        <form method="POST" action="{{ route('admin.project-processes.destroy', $pp) }}" class="js-process-destroy" data-id="{{ $pp->id }}" onsubmit="return confirm('Remover processo deste projeto?');">
                                            @csrf @method('DELETE')
                                            <button class="inline-flex items-center gap-1 px-4 py-2 bg-red-600 text-white rounded text-sm hover:bg-red-700">
                                ...
                                <form method="POST" action="{{ route('admin.project-processes.update', $pp) }}" class="grid grid-cols-12 gap-3 items-start mb-3 js-process-update" data-id="{{ $pp->id }}">
                                    @csrf @method('PUT')
                                    ...
                                    <div class="col-span-1">
                                        ...
                                        <input type="number" name="order" value="{{ $pp->order }}" class="w-full border-gray-300 rounded-md text-sm">
                                    </div>
                                    <div class="col-span-1 flex items-end">
                                        <button class="inline-flex items-center justify-center px-4 py-3 bg-orange-600 text-white rounded border border-transparent hover:bg-white hover:text-orange-600 hover:border-orange-600 text-sm" title="Salvar">
                                            <i class="fa-solid fa-rotate-right"></i>
                                        </button>
                                    </div>
                                </form>
                                ...
                                <div id="pp-{{ $pp->id }}-images" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                        @foreach ($pp->images as $img)
                                            <div id="img-card-{{ $img->id }}" class="border rounded overflow-hidden">
                                                <img src="{{ asset($img->image) }}" class="w-full h-28 object-cover cursor-pointer" onclick="openUploadModal('edit-img-{{ $img->id }}')">
                                            ...
                                            <div id="edit-img-{{ $img->id }}" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
                                                <div class="bg-white rounded shadow-lg w-full max-w-lg p-6">
                                                    ...
                                                    <form id="img-form-{{ $img->id }}" method="POST" action="{{ route('admin.project-images.update', $img) }}" class="space-y-3 js-image-update" data-img-id="{{ $img->id }}">
                                                        @csrf @method('PUT')
                                                        <div>
                                                            <label class="block text-sm font-medium text-gray-700 mb-1">Título</label>
                                                            <input type="text" name="title" value="{{ $img->title }}" class="w-full border-gray-300 rounded-md text-sm">
                                                        </div>
                                                        <div>
                                                            <label class="block text-sm font-medium text-gray-700 mb-1">Descrição</label>
                                                        ...
                                                        <div>
                                                            ...
                                                            <select name="solution" class="w-full border-gray-300 rounded-md text-sm">
                                                                ...
                                                                @foreach ($project->solutions as $sol)
                                                                    <option value="{{ $sol->title }}" @selected($img->solution === $sol->title)>{{ $sol->title }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div>
                                                            <label class="block text-sm font-medium text-gray-700 mb-1">Ordem</label>
                                                            <input type="number" name="order" value="{{ $img->order }}" class="w-full border-gray-300 rounded-md text-sm">
                                                        </div>
                                                    </form>
                                                    <div class="flex items-center justify-between mt-4">
                                                        ...
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <form class="js-image-destroy" method="POST" action="{{ route('admin.project-images.destroy', $img) }}" onsubmit="return confirm('Excluir esta imagem?');">
                                                                @csrf @method('DELETE')
                                ...
                                <div id="upload-pp-{{ $pp->id }}" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
                                    <div class="bg-white rounded shadow-lg w-full max-w-lg p-6">
                                        ...
                                        <form method="POST" action="{{ route('admin.project-processes.images.store', $pp) }}" enctype="multipart/form-data" class="space-y-4 js-images-store" data-pp-id="{{ $pp->id }}">
                                            @csrf
                    ...
                    <div class="bg-white p-4 rounded shadow-sm border border-gray-100 mb-4">
                        <form id="attachSkillsForm" method="POST" action="{{ route('admin.projects.skills.attach', $project) }}" class="space-y-3">
                            @csrf
                            <div class="grid grid-cols-12 gap-3 items-end">
                                <div class="col-span-6">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Skill</label>
                                    <select id="skillSelect" name="skill_id" class="w-full border-gray-300 rounded-md text-sm" required>
                                        <option value="">Selecione...</option>
                            ...
                            <div class="flex justify-end">
                                <button class="inline-flex items-center gap-1 px-5 py-3 bg-orange-600 text-white rounded text-sm hover:bg-orange-700">
                    ...
                    <script>
                        ...
                        function renderCompetencies(skillId) {
                            ...
                            (skill?.competencies || []).forEach(c => {
                                ...
                                btn.addEventListener('click', () => {
                                    ...
                                    btn.className = 'px-3 py-1 rounded-full border text-xs ' + (!selected ? 'bg-orange-600 text-white border-orange-600' : 'bg-gray-100 hover:bg-gray-200');
                                    syncInputs();
                                });
                                ...
                                if (res.ok) {
                                    ...
                                    if (grid && Array.isArray(data.images)) {
                                        ...
                                        data.images.forEach(img => {
                                            const opts = solutions.map(s => `<option value="${s.title}">${s.title}</option>`).join('');
                        ...
                        chList?.addEventListener('submit', async (e) => {
                            ...
                            if (form.classList.contains('js-challenge-update')) {
                                e.preventDefault();
                                await fetch(form.action, { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: new FormData(form) });
                            }
