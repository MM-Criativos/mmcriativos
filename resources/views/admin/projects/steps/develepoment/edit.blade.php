@php
    ...
    $skillOptionsCollection = $skillOptions instanceof \Illuminate\Support\Collection ? $skillOptions : collect($skillOptions ?? []);
            ...
            <form id="task-update-{{ $task->id }}" method="POST" action="{{ route('admin.project-tasks.update', $task) }}"
                    ...
                    <select name="skill_id" x-model="skill" @change="ensureCompetency()" required
                        ...
                        @foreach ($skillOptionsCollection as $option)
                            <option value="{{ $option['id'] }}"
                                @selected((string) $initialSkillValue === (string) $option['id'])>{{ $option['name'] }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach ($statusOptions as $value => $label)
                            <option value="{{ $value }}" @selected($initialStatusValue === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full border-gray-300 rounded-md text-sm focus:border-orange-500 focus:ring-orange-500">{{ $initialDescriptionValue }}</textarea>
                ...
                <div class="md:col-span-2 space-y-4">
                    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Atualize, inclua ou remova subtarefas sem sair deste modal.</p>
                        </div>
                    ...
                    <template x-if="!items.length">
                        <div class="border border-dashed border-gray-200 rounded-md p-4 text-sm text-gray-500">
                            Esta tarefa ainda n&atilde;o tem itens cadastrados.
                        </div>
                    </template>
                                    ...
                                    <select :name="`items[${index}][skill_competency_id]`" x-model="item.skill_competency_id"
                                        ...
                                        <template x-for="competencyOption in competencies" :key="competencyOption.id">
                                            <option :value="String(competencyOption.id)" x-text="competencyOption.name"
                                                :selected="String(item.skill_competency_id) === String(competencyOption.id)"></option>
                                        </template>
                                    </select>
