...
<div class="mt-6">
    ...
    @if(!$hasResponses)
        <div class="mb-4 flex items-center justify-between">
            <p class="text-sm text-gray-600">Este formulário é enviado ao cliente. Use o botão ao lado para enviar o link seguro por e‑mail.</p>
                            ...
                            <div x-show="open" x-collapse>
                                <div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    @foreach($categoryReguas as $regua)
                                        @php
                                            $response = $responses->firstWhere('briefing_regua_id', $regua->id);
                                            if (!$response || $response->value == 0) continue;
                                        ...
                                        <div class="bg-gray-50 rounded-lg p-4">
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    @if($response->comment)
                                                        <p class="mt-1 text-sm text-gray-600">{{ $response->comment }}</p>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
