...
@php
    ...
    $initialSkill = old('skill_id', '');
    $initialCompetency = old('skill_competency_id', '');
    $initialProject = old('project_id', '');
    $initialStatus = old('status', \App\Models\ProjectTask::STATUS_PENDING);
    $initialAssigned = old('assigned_to', '');
    $initialTitle = old('title', '');
    $initialDescription = old('description', '');
    $plannedAtRaw = old('planned_at', '');
    if ($plannedAtRaw) {
        try {
            $plannedAtValue = \Carbon\Carbon::parse($plannedAtRaw)->format('d/m/Y H:i');
        } catch (\Throwable $exception) {
            $plannedAtValue = $plannedAtRaw;
        }
    } else {
        $plannedAtValue = '';
    }
...
<div x-cloak x-show="createTaskModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
    <div class="absolute inset-0 bg-black bg-opacity-40" @click="createTaskModal = false"></div>

    <div class="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl dark:bg-neutral-900"
        ...
        <div class="flex items-start justify-between px-6 py-4 border-b border-gray-200 dark:border-neutral-800">
            <div>
                <h3 class="text-lg font-semibold  text-gray-900 dark:text-white">Nova tarefa</h3>
        ...
        <div class="p-6 space-y-6">
            @if ($skills->isEmpty())
                <div class="border border-dashed border-orange-300 rounded-lg bg-orange-50 text-orange-700 text-sm p-4">
                    Cadastre skills e competências no módulo de Skills para liberar o cadastro de tarefas.
                </div>
            @else
                <form method="POST" action="{{ route('admin.tasks.calendar.store') }}" class="space-y-6"
                    x-data="{
                        options: @js($skillOptions),
                        skill: @js($initialSkill),
                        competency: @js($initialCompetency),
                        project: @js($initialProject),
                    ...
                    @csrf
                    <input type="hidden" name="_calendar_task_form" value="1">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="status"
                                ...
                                @foreach ($statusOptions as $value => $label)
                                    <option value="{{ $value }}" @selected($initialStatus === $value)>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            @error('status')
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Planejado
                                para</label>
                            <input id="plannedAt" type="date" name="planned_at" data-calendar-datetime
                                value="{{ $plannedAtValue }}" placeholder="dd/mm/aaaa"
                                class="w-full dark:!bg-[#262626] border-gray-300 rounded-md text-sm focus:border-orange-500 focus:ring-orange-500">
                            @error('planned_at')
                                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="space-y-4">
                        ...
                        <template x-if="!items.length">
                            <div class="border border-dashed border-gray-200 rounded-md p-4 text-sm text-gray-500">
                                Nenhum item adicionado. Clique em “Adicionar item” para criar o primeiro subtarefa.
                            </div>
                        </template>

                        <template x-for="(item, index) in items" :key="item.uid">
                            <div class="border border-gray-200 rounded-lg p-4 space-y-3 dark:bg-[#262626]">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs  text-gray-500 mt-1">Deixe em branco para
                                            herdar a competência
