<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Classe</h2>
                    ...
                    <form method="POST" action="{{ route('admin.classes.update', $classe) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="classe" value="{{ old('classe', $classe->classe) }}" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Hierarquia</label>
                                <select name="hierarquia" class="mt-1 block w-full border-gray-300 rounded-md">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="Descrição da classe">{{ old('description', $classe->description) }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Skills (separe por vírgula ou quebra de linha)</label>
                            @php $skillsValue = is_array($classe->skills) ? implode(', ', $classe->skills) : (string) $classe->skills; @endphp
                            <textarea name="skills" rows="3" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="Ex.: HTML, CSS, Laravel">{{ old('skills', $skillsValue) }}</textarea>
                        </div>
