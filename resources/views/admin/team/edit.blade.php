<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.team.update', $user) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <textarea name="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="Breve bio do integrante">{{ old('description', $user->description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="cargo" value="{{ old('cargo', $user->cargo) }}" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="Ex.: Desenvolvedor, Designer...">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Foto</label>
                                <div class="relative group cursor-pointer w-40 h-40">
                                    <input type="file" name="photo" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer z-10" onchange="previewImage(event, 'user-photo')">
                                    @php $photo = $user->photo ? asset($user->photo) : null; @endphp
                                    @if ($photo)
                                        <img id="preview-user-photo" src="{{ $photo }}" alt="Foto" class="w-40 h-40 object-cover rounded border border-gray-200 group-hover:opacity-80 transition">
                                    @else
                                        <div id="preview-user-photo" class="flex items-center justify-center w-40 h-40 border border-dashed border-gray-300 rounded bg-gray-50 text-gray-400 text-xs text-center group-hover:bg-orange-50">
                        ...
                        <div class="space-y-4">
                            @foreach ($socialMedias as $media)
                                @php
                                    $existing = optional($user->socialMedias->firstWhere('id', $media->id))->pivot->url ?? '';
                                @endphp
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                                    ...
                                    <div class="md:col-span-2">
                                        <input type="url" name="socials[{{ $media->id }}]" value="{{ old('socials.' . $media->id, $existing) }}" placeholder="https://..." class="mt-1 block w-full border-gray-300 rounded-md">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div id="classBadges" class="space-y-4 mb-2">
                            ...
                            @foreach ([1,2,3] as $level)
                                ...
                                @if ($items->isNotEmpty())
                                    <div>
                                        <div class="text-sm font-semibold text-gray-600 mb-2">Hierarquia {{ $level }}</div>
    ...
    <script>
        function previewImage(evt, id) {
            ...
            el.outerHTML = `<img id=\"preview-user-photo\" src=\"${url}\" class=\"w-40 h-40 object-cover rounded border border-gray-200\" />`;
        }
        ...
        (function () {
            ...
            badges?.querySelectorAll('button[data-id]')?.forEach(btn => {
                ...
                btn.addEventListener('click', () => {
                    ...
                    btn.className = 'px-3 py-1 rounded-full border text-xs ' + (!sel ? 'bg-orange-600 text-white border-orange-600' : 'bg-gray-100 hover:bg-gray-200');
                    sync();
                });
            });
            sync();
