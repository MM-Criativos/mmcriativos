                    ...
                    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select id="client_id" name="client_id" class="mt-1 block w-full border-gray-300 rounded-md" required>
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->id }}" @selected(old('client_id', $testimonial->client_id) == $client->id)>{{ $client->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="contact_id" name="contact_id" class="mt-1 block w-full border-gray-300 rounded-md" required>
                                    @foreach ($contacts as $contact)
                                        <option value="{{ $contact->id }}" @selected(old('contact_id', $testimonial->contact_id) == $contact->id)>{{ $contact->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title', $testimonial->title) }}" class="mt-1 block w-full border-gray-300 rounded-md">
                            </div>
                            <div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-orange-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-orange-700">Salvar</button>
                        </div>
                    </form>
                </div>
            </div>
