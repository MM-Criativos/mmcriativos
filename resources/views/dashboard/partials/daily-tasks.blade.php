<div id="daily-tasks-section" class="flex flex-col gap-4">
    ...
    @else
        @foreach ($dailyTasks as $task)
            @php
                $isLate = $task->planned_at && $task->planned_at->isBefore($selectedDate) && $task->status !== \App\Models\ProjectTask::STATUS_DONE;
            ...
            @endphp
            <div class="flex items-center justify-between border-l-4 {{ $borderClass }} pl-4 py-2 bg-neutral-900">
                <div>
                    <h6 class="text-base font-semibold {{ $isLate ? 'text-red-400' : 'text-white' }}">
                        {{ $task->title }}
                    </h6>
